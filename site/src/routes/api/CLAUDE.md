...
| GET | `/api/my/characters/versions` | `MyCharacterVersionsResponse` (JSON) — `{ characters: (Character & { versions: CharacterVersionFull[] })[] }` where each version's `skills`/`items`/`implants` are joined with the catalog: `skills: { id, name, group, groupName, value }[]`, `items: { id, name, description, count }[]`, `implants: { id, name, description }[]` | List the current user's characters with their versions, each version's skill/item/implant IDs resolved to full catalog entries |
