<script lang="ts">
  ...
  const ALL_QUESTIONS = questionsData as {
    text: string;
    weights: Partial<Record<Company, number>>;
  }[];
  const QUIZ_SIZE = 10;
  ...
  let questions = $state<
    { text: string; weights: Partial<Record<Company, number>> }[]
  >([]);
  ...
  function submitQuiz() {
    ...
    answers.forEach((answer, i) => {
      ...
      for (const [company, weight] of Object.entries(weights) as [
        Company,
        number,
      ][]) {
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    if (phase === "quiz") {
      if (e.key === "ArrowUp" || e.key === "ArrowDown") {
        e.preventDefault();
        const currentIdx =
          selectedAnswer === null ? -1 : answerValues.indexOf(selectedAnswer);
        if (e.key === "ArrowUp") {
        ...
        if (num >= 1 && num <= 5) selectAnswer(ANSWERS[num - 1].value);
      }
    }
  }
</script>
